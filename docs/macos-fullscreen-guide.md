...
`flux-station` scenario 另外驗證 dedicated Flux Station texture、world-station availability、
exact animated Creating Flux card bounds、bedrock/`fluxnetworks:flux_block` base explanation、
1 redstone → 1 flux dust，以及 storage transaction 不修改 world block/entity；這些也必須由
使用者完成 current F11 visual verdict。
