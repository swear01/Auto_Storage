...
Transform is a deterministic resource-conversion shortcut, not a machine inventory and not an automatic background job. It has one server-owned transient input that remains visibly in the upper slot, the persistent searchable Auto/exact target sidebar defined by GitHub #23, exact-input use cards, and the shared upper ×1/×8/×64/Max strip. Direct Fuel cards omit redundant source text; timed cards name their required station/work inline. Auto discovers every compatible use but never preselects or consumes one. Closing or leaving the page returns every unconverted input.
...
Processing receives more than half of the station workspace and up to three readable columns. Its aggregate installed count is the item overlay; the adjacent value is accumulated workstation work. Its fixed column rhythm stays left/top aligned when an incomplete final row has one or two entries. Instant uses the remaining width as a dense icon grid because it has no separate work value. Both grids derive rows from the available height rather than a fixed row target.

Processing uses a representative item with installed aggregate decoration plus horizontal work; Instant uses only compact representative items. Both groups keep explicit previous/next controls and panel-local wheel paging. Their blank top-header search field shows no placeholder; a non-empty query replaces both groups with one result grid containing stations only, and clearing it restores the two paged groups. The inventory-side panel stays on type capacity and never becomes a duplicate hover summary. Station item hitboxes retain the normal item tooltip; Processing value hitboxes show only the localized value name and current aggregate rate. Shift-click routing uses the exact descriptor regardless of the current group page. A dim representative behind an empty real slot explains what belongs there without pretending it is installed.
