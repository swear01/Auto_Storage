...
`src/apiTest/` proves both factories, the explicit-eligibility overload, and the resource kind/handler/block/container APIs compile from a different Java package. `src/recipeAddonFixture/` is a separately loaded NeoForge mod with 17 GameTests covering the public transaction boundary. Each bundled module keeps its authoritative focused fixture and joins the descriptor-derived aggregate. `src/compatibilityMatrixFixture/` adds three GameTests over every descriptor-owned representative optional mod plus the reject-only companion: two verify registries, resource kinds, accepted families, shared descriptors, and fail-closed boundaries; the third first verifies per-module, companion, and unclaimed recipe-inventory SHA-256 values, then benchmarks 10,000 exact stored variants by default or 30,000 with `-PterminalScaleTypes=30000`. It writes load/save/interaction/heap/packet/persistence evidence to `build/reports/terminal-scale-<types>.json`; a recipe-count change cannot pass by preserving only a global total. Public handler/strategy simulation and amount rules are authoritative in [`typed-resource-storage.md`](typed-resource-storage.md#public-handler-and-strategy-contract).
