...
- **Immersive Engineering:** present-mod CI accepts deterministic Arc Furnace and Bottling Machine subsets through master-block stations. Sawmill, Crusher, Alloy Smelter, and Metal Press remain fail closed because sawblade/fuel/mold/tag-output state is not represented. Stone multiblocks, live multipliers, chance secondaries, Cloche randomness, and `TagOutput` preference resolution remain fail closed; see [`immersiveengineering-compatibility.md`](immersiveengineering-compatibility.md).
