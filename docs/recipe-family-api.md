...
- **Oritech:** exact `OritechRecipe` holders of type `oritech:pulverizer` consume simple item inputs plus live `energyPerTick × time` FE and station work, emit `results[0]` as primary and further results as deterministic remainders (identical component-exact keys merge), and derive pulverizer throughput from loaded Oritech config. Other shared-class RecipeContent types, nonempty fluid outputs, oversized ingredient+FE layouts, and addon machine state fail closed; see [`oritech-compatibility.md`](oritech-compatibility.md).
