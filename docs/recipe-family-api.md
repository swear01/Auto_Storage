...
- **Create Crafts & Additions:** exact `RollingRecipe` and `ChargingRecipe` with Rolling Mill / Tesla Coil Processing stations, config-backed work, exact FE consume for charging, crafting remainders, and Liquid Burning fail-closed; see [`createaddition-compatibility.md`](createaddition-compatibility.md).
