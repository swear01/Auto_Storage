package com.swear.autostorage.fixture.justdirethings;
...
@GameTestHolder(JustdirethingsFixtureMod.MODID)
@PrefixGameTestTemplate(false)
public final class JustdirethingsIntegrationGameTests {
    private JustdirethingsIntegrationGameTests() {
    }
    ...
    public static void present_mod_registers_no_unsafe_families(GameTestHelper helper) {
        IsolatedRecipeInventoryEvidence.assertMatchesDescriptor(
                ...
                JustdirethingsIntegrationGameTests.class);
        if (!ModList.get().isLoaded("justdirethings")
                || AutoStorage.RECIPE_FAMILY_REGISTRY.keySet().stream()
                        .anyMatch(id -> id.getNamespace().equals("justdirethings")
                                || id.getPath().startsWith("justdirethings_"))
                || AutoStorage.MACHINE_DESCRIPTOR_REGISTRY.keySet().stream()
                        .filter(id -> id.getNamespace().equals("justdirethings")
                                || id.getPath().startsWith("justdirethings_"))
                        .anyMatch(id -> !id.equals(GENERATOR))) {
            helper.fail("Just Dire Things unsafe machine contract was registered");
            return;
        }
        helper.succeed();
    }
    ...
    public static void vanilla_crafting_under_namespace_stays_supported(
        ...
        var holder = helper.getLevel().getRecipeManager()
                .byKey(jdt("generatort1")).orElse(null);
        if (holder == null
                || !CraftingTerminalMenu.supportsRecipeHolder(holder)) {
            helper.fail("Just Dire Things vanilla crafting must stay supported");
            return;
        }
        helper.succeed();
    }
    ...
    public static void transform_ingredient_shortage_is_atomic(GameTestHelper helper) {
        withCore(helper, (level, core, player) -> {
            ...
            installStation(core, player, jdtItem("generatort1"));
            menu.getSlot(CraftingTerminalMenu.FUEL_INPUT_SLOT)
                    .set(ItemStack.EMPTY);
            long work = generatorWork(new ItemStack(Items.COAL));
            addCoreTicks(core, Math.toIntExact(work));
            ...
            if (clicked
                    ...
                    || accrued <= 0) {
                helper.fail(
                        "Just Dire Things missing-ingredient transform was not an atomic no-op");
                return;
            }
            helper.succeed();
        });
    ...
    public static void transform_destination_overflow_is_atomic(GameTestHelper helper) {
        withCore(helper, (level, core, player) -> {
            ...
            var use = menu.getTransformUses().stream()
                    ...
                    .orElse(null);
            if (use == null) {
                helper.fail("Just Dire Things generator transform use is missing");
                return;
            }
            selectTransform(menu, player, GENERATOR);
            installStation(core, player, jdtItem("generatort1"));
            ...
            if (clicked
                ...
                helper.fail(
                        "Just Dire Things full destination transform was not an atomic no-op");
                return;
            }
            helper.succeed();
        });
    ...
    @GameTest(template = "craftingtests.platform")
    public static void generator_t1_converts_fuel_to_fe_over_exact_work(
            GameTestHelper helper
    ) {
        withCore(helper, (level, core, player) -> {
            var menu = transformMenu(core, player, new ItemStack(Items.COAL));
            long work = generatorWork(new ItemStack(Items.COAL));
            long expected = coalFe();
            var use = menu.getTransformUses().stream()
                    .filter(candidate -> candidate.id().equals(GENERATOR))
                    .findFirst()
                    .orElse(null);
            ...
            if (menu.clickMenuButton(player, 2)
                    || core.getResourceAmount(
                            StorageResourceKey.neoforgeEnergy()) != 0) {
                helper.fail("Just Dire Things generator must reject without an installed generator");
                return;
            }
            installStation(core, player, jdtItem("generatort1"));
            addCoreTicks(core, Math.toIntExact(work));
            if (!menu.clickMenuButton(player, 2)
                ...
                helper.fail(
                        "Just Dire Things generator committed the wrong FE/work/input transaction");
                return;
            }
            helper.succeed();
        });
    ...
    private static long generatorWork(ItemStack fuel) {
        return fuel.getBurnTime(RecipeType.SMELTING)
                / Config.GENERATOR_T1_BURN_SPEED_MULTIPLIER.get();
    }

    private static long coalFe() {
        return 1600L * Config.GENERATOR_T1_FE_PER_FUEL_TICK.get();
    }
    ...
    private static final ResourceLocation GENERATOR =
            ResourceLocation.fromNamespaceAndPath(
                    AutoStorage.MODID, "justdirethings_generator_t1");
    ...
    private static void installStation(
        ...
        menu.clickMenuButton(player, STORAGE_PAGE_BUTTON);
        throw new IllegalStateException("Could not install Just Dire Things station");
    }
    ...
    private static ResourceLocation jdt(String path) {
        return ResourceLocation.fromNamespaceAndPath("justdirethings", path);
    }

    private static Item jdtItem(String path) {
        Item item = BuiltInRegistries.ITEM.get(jdt(path));
        if (item == Items.AIR) {
            throw new IllegalStateException("Missing Just Dire Things item " + path);
        }
        return item;
    }
