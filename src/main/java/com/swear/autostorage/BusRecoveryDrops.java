...
final class BusRecoveryDrops {
    private static final String TAG_RECOVERY_DROP_ID = "autoStorageRecoveryDropId";
    ...
    private static boolean hasEscrowPayload(ItemStack stack) {
        if (!stack.is(AutoStorage.IMPORT_BUS_ITEM.get())
                && !stack.is(AutoStorage.EXPORT_BUS_ITEM.get())) {
            return false;
        }
