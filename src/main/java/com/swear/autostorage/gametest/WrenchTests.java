...
public class WrenchTests {
    ...
    public static void own_wrench_is_registered_and_uses_common_tag(GameTestHelper helper) {
        ...
        var wrench = registry.get(ResourceLocation.fromNamespaceAndPath(AutoStorage.MODID, "wrench"));
        if (wrench == null || wrench != AutoStorage.WRENCH.get()) {
    ...
    public static void wrench_rotates_both_directional_buses(GameTestHelper helper) {
        ...
        level.setBlock(importPos,
                AutoStorage.IMPORT_BUS.get().defaultBlockState().setValue(ImportBusBlock.FACING, Direction.NORTH),
                Block.UPDATE_ALL);
        level.setBlock(exportPos,
                AutoStorage.EXPORT_BUS.get().defaultBlockState().setValue(ExportBusBlock.FACING, Direction.NORTH),
                Block.UPDATE_ALL);

        var player = helper.makeMockPlayer(GameType.SURVIVAL);
        player.setPos(importPos.getX() + 0.5, importPos.getY() + 0.5, importPos.getZ() + 0.5);
        player.setItemInHand(InteractionHand.MAIN_HAND, new ItemStack(AutoStorage.WRENCH.get()));
        ...
        var importState = AutoStorage.IMPORT_BUS.get().defaultBlockState()
        ...
        var exportState = AutoStorage.EXPORT_BUS.get().defaultBlockState()
    ...
    public static void directionless_bus_normal_wrench_passes_without_rotation(
        ...
        var importPos = helper.absolutePos(new BlockPos(1, 3, 1));
        var exportPos = helper.absolutePos(new BlockPos(3, 3, 1));
        level.setBlock(importPos, AutoStorage.IMPORT_BUS.get().defaultBlockState()
                .setValue(ImportBusBlock.FACING, Direction.NORTH), Block.UPDATE_ALL);
        level.setBlock(exportPos, AutoStorage.EXPORT_BUS.get().defaultBlockState()
    ...
    public static void bus_wrench_rotation_and_dismantle_require_owner(GameTestHelper helper) {
        ...
        helper.runAfterDelay(2, () -> {
            ...
            owner.setItemInHand(InteractionHand.MAIN_HAND, new ItemStack(AutoStorage.WRENCH.get()));
            intruder.setItemInHand(InteractionHand.MAIN_HAND, new ItemStack(AutoStorage.WRENCH.get()));
            ...
            for (int slot = 0; slot < owner.getInventory().getContainerSize(); slot++) {
                if (owner.getInventory().getItem(slot).is(AutoStorage.IMPORT_BUS_ITEM.get())) {
            ...
            for (ItemEntity entity : level.getEntitiesOfClass(
                ...
                if (entity.getItem().is(AutoStorage.IMPORT_BUS_ITEM.get())) entity.discard();
            }

            BlockPos canceledBusPos = corePos.north();
            level.setBlock(
                    canceledBusPos, AutoStorage.IMPORT_BUS.get().defaultBlockState(), Block.UPDATE_ALL);
            ...
            for (ItemStack stack : owner.getInventory().items) {
                if (stack.is(AutoStorage.IMPORT_BUS_ITEM.get())) conserved += stackEscrow(stack);
            }
            for (ItemEntity entity : level.getEntitiesOfClass(
                    ItemEntity.class, new AABB(canceledBusPos).inflate(2.0))) {
                if (entity.getItem().is(AutoStorage.IMPORT_BUS_ITEM.get())) {
    ...
    public static void replaced_wrench_drop_event_does_not_restore_plain_bus_without_escrow(
        ...
        for (ItemStack stack : player.getInventory().items) {
            if (stack.is(AutoStorage.IMPORT_BUS_ITEM.get())) busItems += stack.getCount();
        }
        for (ItemEntity entity : level.getEntitiesOfClass(
    ...
    public static void ordinary_wrench_passes_non_directional_blocks_and_rejects_wrong_context(GameTestHelper helper) {
        ...
        spectator.setItemInHand(InteractionHand.MAIN_HAND, new ItemStack(AutoStorage.WRENCH.get()));
    ...
    public static void sneak_wrench_dismantle_preserves_core_inventory_energy_and_identity(GameTestHelper helper) {
        ...
        helper.runAfterDelay(2, () -> {
            ...
            ItemStack droppedCore = findInventoryStack(player.getInventory(), AutoStorage.STORAGE_CORE_ITEM.get());
            ...
            level.setBlock(restoredPos, AutoStorage.STORAGE_CORE.get().defaultBlockState(), Block.UPDATE_ALL);
            level.setBlock(restoredPos.east(), AutoStorage.STORAGE_UNIT_T1.get().defaultBlockState(), Block.UPDATE_ALL);
            AutoStorage.STORAGE_CORE.get().setPlacedBy(
    ...
    public static void full_inventory_dismantle_spawns_exact_world_overflow(GameTestHelper helper) {
        ...
        if (!findInventoryStack(player.getInventory(), AutoStorage.STORAGE_UNIT_T1_ITEM.get()).isEmpty()) {
        ...
        for (ItemEntity entity : level.getEntitiesOfClass(ItemEntity.class, new AABB(unitPos).inflate(2.0))) {
            if (entity.getItem().is(AutoStorage.STORAGE_UNIT_T1_ITEM.get())) {
    ...
    public static void export_bus_drop_preserves_only_component_rich_filter(GameTestHelper helper) {
        ...
        helper.runAfterDelay(2, () -> {
            ...
            for (ItemStack drop : drops) {
                if (drop.is(AutoStorage.EXPORT_BUS_ITEM.get())) {
                    busDrop = drop;
                    break;
                }
            }
            ...
            var restoredPos = helper.absolutePos(new BlockPos(5, 3, 1));
            level.setBlock(restoredPos, AutoStorage.EXPORT_BUS.get().defaultBlockState(), Block.UPDATE_ALL);
