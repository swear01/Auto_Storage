...
public final class TransformProviderApi {
    ...
    public static synchronized void register(
        ...
        PROVIDERS.add(new Provider(id, representative.copyWithCount(1), resolver));
    }
    ...
    public static ResourceLocation energyTargetId(EnergyType type) {
        ...
        return ResourceLocation.fromNamespaceAndPath(MagicStorage.MODID, type.getId());
    }
    ...
    static List<ResourceLocation> targetIds(List<MachineDescriptor> descriptors) {
        ...
        Set<ResourceLocation> unique = new HashSet<>(result);
        if (unique.size() != result.size()) {
            throw new IllegalStateException("Duplicate built-in transform target");
        }
    ...
    static List<Target> targets(List<MachineDescriptor> descriptors) {
        ...
        for (EnergyType type : EnergyType.values()) {
            if (!type.isMachineGenerated()) {
                result.add(new Target(energyTargetId(type), type.representativeStack()));
            }
        }
        descriptors.stream()
                .filter(descriptor -> descriptor.category() == MachineEnergyTable.Category.TRANSFORM)
                .map(descriptor -> new Target(descriptor.id(), descriptor.representativeStack()))
                .forEach(result::add);
        synchronized (TransformProviderApi.class) {
            PROVIDERS.stream()
                    .map(provider -> new Target(provider.id(), provider.representative()))
                    .forEach(result::add);
        }
        return List.copyOf(result);
    }
    ...
    public record Target(ResourceLocation id, ItemStack representative) {
        public Target {
            Objects.requireNonNull(id, "id");
