...
description: "Runs a broad, read-only whole-repository audit for correctness, strange behavior, corner cases, security, performance, concurrency, lifecycle, dependencies, duplication, dead code, tests, CI, and documentation, then publishes exactly one deduplicated GitHub issue. Use for a manual full repo audit rather than a PR diff review."
license: "MIT; adapted from ln-22-codebase-auditor"
compatibility: "Requires repository read access, optional web access, and an authenticated gh CLI token with issues:write to publish the report."
metadata:
  upstream: "https://github.com/levnikolaevich/claude-code-skills"
  upstream-commit: "a8b40c18969b89f5f422389c5ab385c5771a05ea"
---
...
- When design intent or a difficult implementation is unclear, compare similar projects
  and similar modules. Follow project-named references first; for this
  repository, check Refined Storage 2 before broader alternatives.
...
Adapted from `ln-22-codebase-auditor` at the pinned upstream commit in the metadata.
See `references/UPSTREAM_LICENSE.txt`.
