...
- Compat Kit complete contracts must declare target HTTPS Maven repositories, one authoritative GameTest task/count, and `{task, source, marker}` evidence for every required check. `source_recipe_inventory_sha256` must match the exact set of family classes so no audited candidate can be deleted. Verification must confirm the exact source annotation count, exact runtime passing count, marker presence, successful associated task, and resolved target jar SHA before reporting a check passed; evidence task names are never remapped. External addon contracts use fixture `main` and exactly `build` plus `runGameTestServer`; bundled fixture names must match `[a-z][A-Za-z0-9]*Fixture` before materialization, and bundled descriptors must retain the contract's reviewed repositories. Named nested classes are audited and mapped to their top-level source while anonymous/local/synthetic classes are excluded; every changed target artifact SHA forces contract review even when public signatures are stable. Descriptor-generated run names come from the fixture's preserved camel case (`evilCraftFixture` → `runEvilCraftGameTestServer`), never from a lowercased directory ID. Published archives must scaffold independently with their adjacent wrapper template.
