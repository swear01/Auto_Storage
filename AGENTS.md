...
- Compat Kit complete validation, generation, conformance, resource scaffolding, scaffold, and verification must receive the exact target `--jar`, rehash it against the audit, and independently rebuild its complete sorted class/metadata inventory before trusting the audit graph; when the jar is the only recipe-data source, its complete recipe inventory must also be rebuilt and compared, and its source recipe count is always checked even when explicit data roots exist. An audit's self-consistent count/digest is not proof by itself. Generated addon CI must stage that SHA-verified target jar and pass the staged path back through `--jar`; a committed audit file alone cannot satisfy verification. A supplied source checkout may use only an authoritative class-file `SourceFile` mapping; when that attribute is absent, fail with source mapping unavailable instead of guessing `<BinaryName>.java`. The selected `javap` executable itself must run `-version` as major JDK 21, and its resolved path, size, SHA, and reported version belong to the cache identity. GameTest holder string constants are owned by their actual innermost declaring class, never the file stem. Generated Java member bindings must use the shared keyword-aware validator. Worker directives may interpolate only tightly validated identifiers; free-form target display metadata remains escaped, explicitly untrusted JSON evidence. Resource scaffolding must reuse all built-in Item, Fluid, NeoForge Energy, and Work kinds rather than generating duplicate bridges.
