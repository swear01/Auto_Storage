...
class CreativeStorageUnitTests(unittest.TestCase):
    def test_terminal_capacity_display_uses_explicit_unlimited_state(self):
        storage_screen = (ROOT / "src/main/java/com/swear/autostorage/StorageTerminalScreen.java").read_text()
        crafting_screen = (ROOT / "src/main/java/com/swear/autostorage/CraftingTerminalScreen.java").read_text()
        en_us = json.loads((ROOT / "src/main/resources/assets/auto_storage/lang/en_us.json").read_text())
        zh_tw = json.loads((ROOT / "src/main/resources/assets/auto_storage/lang/zh_tw.json").read_text())

        self.assertIn("menu.hasUnlimitedTypeCapacity()", storage_screen)
        self.assertIn('"gui.auto_storage.type_capacity_unlimited"', storage_screen)
        self.assertIn("menu.hasUnlimitedTypeCapacity()", crafting_screen)
        self.assertIn('"gui.auto_storage.type_capacity_unlimited"', crafting_screen)
        self.assertEqual("%s / Unlimited types", en_us["gui.auto_storage.type_capacity_unlimited"])
        self.assertEqual("%s / 無限種類", zh_tw["gui.auto_storage.type_capacity_unlimited"])
    ...
    def test_registration_loot_tag_localization_and_no_recipe(self):
        ...
        self.assertIn('ITEMS.register("creative_storage_unit", () -> new CreativeStorageUnitBlockItem(', auto_storage)
        self.assertIn("output.accept(CREATIVE_STORAGE_UNIT_ITEM.get())", auto_storage)

        en_us = json.loads((ROOT / "src/main/resources/assets/auto_storage/lang/en_us.json").read_text())
        zh_tw = json.loads((ROOT / "src/main/resources/assets/auto_storage/lang/zh_tw.json").read_text())
        self.assertEqual("Creative Storage Unit", en_us["block.auto_storage.creative_storage_unit"])
        self.assertEqual("創造儲存單元", zh_tw["block.auto_storage.creative_storage_unit"])

        pickaxe_tag = json.loads((ROOT / "src/main/resources/data/minecraft/tags/block/mineable/pickaxe.json").read_text())
        self.assertIn("auto_storage:creative_storage_unit", pickaxe_tag["values"])

        loot_path = ROOT / "src/main/resources/data/auto_storage/loot_table/blocks/creative_storage_unit.json"
        ...
        self.assertEqual("auto_storage:creative_storage_unit", loot["pools"][0]["entries"][0]["name"])
    ...
    def test_item_tooltip_localizes_unlimited_capacity_without_item_generation_claims(self):
        item_path = ROOT / "src/main/java/com/swear/autostorage/CreativeStorageUnitBlockItem.java"
        ...
        self.assertIn('Component.translatable("tooltip.auto_storage.creative_storage_unit")', item)
        ...
        self.assertEqual(
            ...
            en_us["tooltip.auto_storage.creative_storage_unit"],
        )
        self.assertEqual(
            "無限物品種類容量；不會生成物品",
            zh_tw["tooltip.auto_storage.creative_storage_unit"],
        )
    ...
    def test_fuel_capacity_uses_creative_icon_only_for_unlimited_state(self):
        ...
        self.assertRegex(
            ...
            re.compile(
                ...
                r"\?\s*AutoStorage\.CREATIVE_STORAGE_UNIT_ITEM\.get\(\)\.getDefaultInstance\(\)\s*"
                r":\s*AutoStorage\.STORAGE_UNIT_T1_ITEM\.get\(\)\.getDefaultInstance\(\)",
                re.DOTALL,
            ),
        )
    ...
    def test_visual_assets_are_complete_and_use_the_storage_family_pipeline(self):
        ...
        visual_paths = [
            resources / "assets/auto_storage/blockstates/creative_storage_unit.json",
            resources / "assets/auto_storage/models/block/creative_storage_unit.json",
            resources / "assets/auto_storage/models/item/creative_storage_unit.json",
            resources / "assets/auto_storage/textures/block/creative_storage_unit.png",
            resources / "resourcepacks/fusion_connected_casing/assets/auto_storage/models/block/creative_storage_unit.json",
            resources / "resourcepacks/fusion_connected_casing/assets/auto_storage/textures/block/creative_storage_unit_connected.png",
            resources / "resourcepacks/fusion_connected_casing/assets/auto_storage/textures/block/creative_storage_unit_connected.png.mcmeta",
        ]
        ...
        self.assertEqual(
            ...
            manifest["members"]["auto_storage:block/creative_storage_unit"]["role"],
        )
        self.assertEqual(
            {"#3FDCE5", "#C083FF"},
